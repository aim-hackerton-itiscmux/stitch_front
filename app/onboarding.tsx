      ...
      <SafeAreaView edges={["top"]} className="flex-1 bg-surface">
        <View className="flex-row items-center justify-between border-b border-surface-container bg-surface-container-lowest px-5 h-16">
          ...
          <Pressable className="p-1">
            <MaterialIcons name="help-outline" size={24} color={stitchColors.primary} />
          </Pressable>
        </View>
        ...
        <ScrollView
          ...
          <View>
            <View className="mb-2 flex-row items-end justify-between">
              <Text className="font-data-label text-data-label text-primary">Step 4 / 5</Text>
              <Text className="font-caption-caps text-caption-caps text-outline">
                기본 조건 입력
              </Text>
            </View>
                ...
                <View className="rounded-lg border border-outline-variant bg-surface">
                  {savingsRanges.map((r, i) => (
                    <Pressable
                      ...
                      className={`px-4 py-3 ${
                        i < savingsRanges.length - 1 ? "border-b border-outline-variant" : ""
                      } ${savingsRangeIdx === i ? "bg-primary-container/10" : ""}`}>
                      <Text
                        className={`font-body-main text-body-main ${
                          savingsRangeIdx === i ? "text-primary" : "text-on-surface"
                        }`}>
                        {r}
                      </Text>
                    </Pressable>
          ...
          <SectionCard icon="group" title="부양가족 수">
            <View className="flex-row items-center justify-between rounded-lg border border-outline-variant bg-surface p-2">
              ...
              <Text
                className="font-report-headline text-report-headline text-on-surface text-center"
                style={{ width: 64 }}>
                {dependents}
              </Text>
              <Pressable
        ...
        <View className="flex-row gap-3 border-t border-surface-container bg-surface-container-lowest px-4 py-4">
          ...
          <Pressable
            ...
            className="flex-[2] items-center rounded-lg bg-primary py-3.5">
            <Text className="font-section-header text-section-header text-on-primary">다음</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    </>
